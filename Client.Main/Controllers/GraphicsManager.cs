    ...
    public abstract class ModelObject : WorldObject
    {
        ...
        public virtual void DrawShadowMesh(int mesh, Matrix view, Matrix projection, GameTime gameTime)
        {
            try
            {
                ...
                //--- 2. Obliczenie macierzy świata dla cienia -------------------------------------------
                Matrix shadowWorld;
                const float shadowBias = 0.1f;      // niewielki offset, aby uniknąć z-fighting-u

                try
                {
                    Vector3 position = WorldPosition.Translation;               // pozycja oryginalnego obiektu
                    float terrainH = World.Terrain.RequestTerrainHeight(position.X, position.Y);
                    terrainH += terrainH * 0.5f;                                // Twój oryginalny „bonus”
                    ...
                    // Pozycja rzutu cienia na teren
                    Vector3 shadowPos = new Vector3(
                        position.X - (heightAboveTerrain / 2),
                        position.Y - (heightAboveTerrain / 4.5f),
                        terrainH + 1f         // minimalne „podniesienie”, żeby nie migotał
                    );

                    // Rotacja cienia (Twój algorytm)
                ...
                //--- 3. Przygotowanie stanów GPU --------------------------------------------------------
                var prevBlendState = GraphicsDevice.BlendState;
                var prevDepthStencilState = GraphicsDevice.DepthStencilState;

                GraphicsDevice.BlendState = Blendings.ShadowBlend;     // SrcAlpha / InvSrcAlpha
                GraphicsDevice.DepthStencilState = DepthStencilState.DepthRead;

                try
                {
                    //--- 4. Ustawienie parametrów nowego shadera ----------------------------------------
